...
FANCONTROL::UpdateTempDisplay(void)
{
	...
	for (int i = 0; i < 12; i++)
	{
		...
		if (show)
		{
			...
			lvi.iItem = i; lvi.iSubItem = 0; lvi.pszText = buf;
			int idx = ListView_InsertItem(hLV, &lvi);
			ListView_SetItemText(hLV, idx, 1, (LPSTR)this->State.SensorName[i]);
			if (temp < 128 && temp != 0) {
				if (Fahrenheit) sprintf_s(buf, sizeof(buf), "%d F", temp * 9 / 5 + 32);
				else sprintf_s(buf, sizeof(buf), "%d C", temp);
			} else strcpy_s(buf, sizeof(buf), "n/a");
